...
The system supports flexible keyboard commands using `+` to combine keys:

- **Single Keys**: `A`, `B`, `C`, `0-9`, etc.
- **Multiple Keys**: `L+N+A+V`, `A+B+C` (pressed simultaneously)
- **With Modifiers**: `CTRL+A`, `SHIFT+B`, `ALT+F4`, `CTRL+SHIFT+S`
- **Function Keys**: `F1` through `F24`
- **Arrow Keys**: `UPARROW`, `DOWNARROW`, `LEFTARROW`, `RIGHTARROW`
- **Special Keys**: `SPACE`, `ENTER`, `TAB`, `ESC`, `DELETE`, etc.
...
- Verify keyboard commands use valid format (e.g., A, CTRL+C, L+N+A+V)

See [Troubleshooting Guide](docs/user-guide.md#troubleshooting) for more solutions.

## Limitations

- **Simultaneous Keys**: Maximum 10 keys per command (can increase in firmware)
...
- [ ] Right modifier keys (currently only left CTRL/SHIFT/ALT/GUI supported)
