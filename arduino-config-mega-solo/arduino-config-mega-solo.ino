/*
 * Arduino Mega 2560 - Solo Configuration (No Pro Micro Required)
 *
 * This version works WITHOUT Arduino Pro Micro.
 * Uses Python keyboard daemon on PC to send keyboard commands.
 *
 * Features:
 ...
 * - Sends Ctrl+ keyboard commands via USB Serial to PC daemon
 *
...
const unsigned long BAUD_RATE = 115200;          // USB Serial baud rate
...
const uint16_t CONFIG_MAGIC = 0xAC02;            // Magic number for config validation (changed from dual-board version)
...
struct InputConfig {
  ...
  char keyCommand[16];   // Keyboard command (e.g., "CTRL+F")
...
void setup() {
  ...
  Serial.println(F("Arduino Mega Solo Input Handler Ready"));
  Serial.println(F("Send JSON configuration to update settings"));
  Serial.println(F("Keyboard commands will be sent to daemon via Serial"));
...
void sendKeyCommand(const char* command) {
  // Send command via USB Serial to Python daemon on PC
...
void sendStatus() {
  ...
  doc["status"] = "ok";
  doc["mode"] = "solo";  // Indicate this is solo mode (no Pro Micro)
